...
class TriangleDataset():
    ...
    def scms_lin(self, U: np.ndarray) -> np.ndarray:
        X = [0.0] * 3
        X[0] = U[0]
        X[1] = U[1] - X[0]
        X[2] = X[0] + 0.25 * X[1] + U[2]
        return np.array(X, dtype=float)


    def scms_nlin(self, U: np.ndarray) -> np.ndarray:
        X = [0.0] * 3
        X[0] = U[0]
        X[1] = U[1] - 1 + 3.0 / (1 + math.exp(-2 * X[0]))
        X[2] = X[0] + 0.25 * (X[1] ** 2) + U[2]
        return np.array(X, dtype=float)


    def scms_nadd(self, U: np.ndarray) -> np.ndarray:
        X = [0.0] * 3
        X[0] = U[0]
        X[1] = 0.25 * (X[0] ** 2) * np.sign(U[1]) * (1 + U[1] ** 2)
        X[2] = -1 + 0.1 * np.sign(U[2]) * (X[1] ** 2) + U[2]
        return np.array(X, dtype=float)

    def sample_U(self, n_samples: int, variant: str) -> np.ndarray:
        if variant != "NADD":
            U0 = 0.5 * np.random.normal(loc=-2, scale=1.5, size=n_samples) + 0.5 * np.random.normal(loc=1.5, scale=1, size=n_samples)
        else:
            U0 = 0.5 * np.random.normal(loc=-2.5, scale=1, size=n_samples) + 0.5 * np.random.normal(loc=2.5, scale=1, size=n_samples)
        if variant == "LIN":
            U1 = np.random.normal(loc=0, scale=1, size=n_samples)
        elif variant == "NLIN":
            U1 = np.random.normal(loc=0, scale=0.1, size=n_samples)
        else:
            U1 = np.random.normal(loc=0, scale=0.25, size=n_samples)
        if variant != "NADD":
            U2 = np.random.normal(loc=0, scale=1, size=n_samples)
        else:
            U2 = np.random.normal(loc=0, scale=0.0625, size=n_samples)
        return np.stack([U0, U1, U2], axis=-1)


    def generate_observational(self, n_samples: int, variant: str = "LIN") -> Tuple[np.ndarray, np.ndarray]:
        """Generate observational dataset X_obs (n_samples x 5) and corresponding U_obs."""
        variant = variant.upper()
        U = self.sample_U(n_samples, variant)
        X = np.zeros((n_samples, 3), dtype=float)
        for i in range(n_samples):
            if variant == "LIN":
                X[i] = self.scms_lin(U[i])
            elif variant == "NLIN":
                X[i] = self.scms_nlin(U[i])
            elif variant == "NADD":
                X[i] = self.scms_nadd(U[i])
            else:
                raise ValueError("variant must be one of 'LIN', 'NLIN', 'NADD'")
        return X, U


    def do_intervention(self, alpha: float, intervene_index: int, variant: str = "LIN", n_samples: int = 1) -> np.ndarray:
        """
        ...
        for i in range(n_samples):
            ...
            if variant == "LIN":
                X[1] = alpha if intervene_index == 1 else U[1] - X[0]
                X[2] = X[0] + U[2]
            elif variant == "NLIN":
    ...
    def do_intervention_counterfactual(self, alpha: float, intervene_index: int, U: np.ndarray, variant: str = "LIN") -> np.ndarray:
        """
        ...
        if variant == "LIN":
            ...
            X[1] = alpha if intervene_index == 1 else U[1] - 1 + 3.0 / (1 + math.exp(-2 * X[0]))
            X[2] = X[0] + 0.25 * (X[1] ** 2) + U[2]
        else:  # NADD
            X[1] = alpha if intervene_index == 1 else 0.25 * (X[0] ** 2) * np.sign(U[1]) * (1 + U[1] ** 2)
            X[2] = -1 + 0.1 * np.sign(U[2]) * (X[1] ** 2) + U[2]
        return np.array(X, dtype=float)


def create_dataset_triangle(n_samples: int, n_obs: int = 100, variant: str = "LIN", counterfactuals: bool = True, seed: int = 42):
    """
    ...
    dataset_path = OtherExperimentsPath.get_triangle_dataset_path(variant, seed)
    if os.path.exists(dataset_path):
        samples = torch.load(dataset_path)
        return samples
    n_samples = n_samples // (5 * len(ds.non_leaf_nodes))
    X_obs, U_obs = ds.generate_observational(n_samples, variant=variant)
    ...
    for i in range(n_samples):
        ...
        for j in ds.non_leaf_nodes:
            for alpha in intervention_values[j]:
                if counterfactuals:
                    x_int = ds.do_intervention_counterfactual(alpha=float(alpha), intervene_index=j, U=u_orig, variant=variant)
                else:
                    x_int = ds.do_intervention(alpha=float(alpha), intervene_index=j, variant=variant, n_samples=1)[0]
                ...
                obs, _ = ds.generate_observational(n_obs, variant=variant)
...
if __name__ == "__main__":
    ...
    for var in ["LIN", "NLIN", "NADD"]:
        ...
        samples_var = create_dataset_triangle(100, n_obs=n_obs_small, variant=var, counterfactuals=True)
        # print first example
