...
class GNNCFNPSeq(nn.Module):
    def __init__(self, in_features, hidden_dim, num_nodes, num_obs,
                 num_count, iterations=3, mog_comp=3, num_heads_att=5, edge_fn=None):
        super().__init__()
        ...
        self.sequential_module = GNNAbductionSeq(
            hidden_dim, num_nodes, num_obs, num_count,
            mog_comp, num_heads_att=num_heads_att, edge_fn=edge_fn
        )

        self.final_layer = MoGLayer(self.hidden_dim, mog_comp=mog_comp, data_dim=in_features)

    def forward(self, batch):
        interventional_indices = batch.int_indices.reshape(
            (batch.batch_size, self.num_count, -1)
        ).unsqueeze(-1)
        ...
        embedded_ast_nonint = self.counterfactual_embedding(x_ast)
        embedded_obs = self.obs_embedding(obs)
        embedded_z_nonint = self.cond_embed(z)

        gather_idx = interventional_indices.expand(-1, -1, -1, self.in_features)

        embedded_ast_int = self.counterfactual_embedding_int(
            x_ast.gather(2, gather_idx)
        )
        embedded_z_int = self.cond_embedding_int(
            z.gather(2, gather_idx)
        )

        embedded_ast = embedded_ast_nonint.scatter(
            2,
            interventional_indices.expand(-1, -1, -1, self.hidden_dim),
            embedded_ast_int
        )

        embedded_z = embedded_z_nonint.scatter(
            2,
            interventional_indices.expand(-1, -1, -1, self.hidden_dim),
            embedded_z_int
        )
        ...
        batch.connection_element = torch.zeros(
            (batch.batch_size, 1, self.num_nodes, self.hidden_dim)
...
class GNNAbductionSeq(nn.Module):
    def __init__(
        self,
        hidden_dim: int,
        num_nodes: int,
        ...
        self.obs_abduction_edges = self.edge_fn(num_nodes, num_obs)
        ...
        self.parallel_dmsm = ParallelDenseGCN(
            ...
            double_num_layers=num_nodes,
            in_dim=self.hidden_dim + self.int_indice_dim,
            hidden_dim=self.hidden_dim
        )

        self.local_dmsm = GCNConv(
            in_channels=self.hidden_dim + self.int_indice_dim,
    ...
    def forward(self, batch):
        ...
        int_indices = batch.int_indices.reshape((batch.batch_size, 1, -1)).expand(-1, embedded_ast.shape[1], -1)
        int_onehot = F.one_hot(int_indices, self.num_nodes).sum(dim=-2).float()
        int_onehot = self.int_indice_layer(int_onehot)

        relevant = torch.cat([
            embedded_obs,
            torch.zeros((batch.batch_size, 1, self.num_nodes, self.hidden_dim), device=embedded_obs.device),
            # embedded_z # This does not match the edges I think. The obs have to be connected to the z nodes later.
        ], dim=1).reshape(batch.batch_size, -1, self.hidden_dim)

        new_edges, z_edges = self.compute_edges(batch, embedded_obs, relevant)
        ...
        # conn = obs_reshaped[:, embedded_obs.shape[1]:-embedded_z.shape[1]]
        conn = obs_reshaped[:, embedded_obs.shape[1]:]

        # z_input = torch.cat([embedded_z, batch.connection_element], dim=1)
        ...
        ast_pos = torch.cat([
            embedded_ast,
            int_onehot.reshape(batch.batch_size, -1, self.num_nodes, self.int_indice_dim)
        ], dim=-1)

        concatted = torch.cat([
            self.conn_matching_layer(conn),
            ast_pos
        ], dim=2).reshape(-1, self.hidden_dim + self.int_indice_dim)
        ...
        concatted = gat_out.reshape(batch.batch_size, -1, 2 * self.num_nodes, self.hidden_dim)
        embedded_ast = concatted[:, :, -self.num_nodes:]
    ...
    def compute_edges(self, batch, embedded_obs, relevant):
        """
        ...
        """
        # add_idx = torch.arange(batch.batch_size).repeat(self.obs_abduction_edges.shape[1]) * relevant.shape[1]
        # new_edges = self.obs_abduction_edges.repeat(1, batch.batch_size) + add_idx
        # new_edges = new_edges.to(device=embedded_obs.device)

        # numbered_nodes = torch.arange(self.num_nodes * 2)
        # z_edges = torch.stack([numbered_nodes % self.num_nodes, numbered_nodes]).repeat(1, batch.batch_size)
        # offsets = torch.arange(batch.batch_size) * self.num_nodes * 2
        # z_edges += offsets.repeat_interleave(self.num_nodes * 2)
        # z_edges = z_edges.to(device=embedded_obs.device)
        # return new_edges,z_edges
        ...
        numbered_nodes = torch.arange(self.num_nodes * 2, device=device)
        z_edges = torch.stack([numbered_nodes % self.num_nodes, numbered_nodes])
        z_edges = z_edges.repeat(1, batch.batch_size)

        z_offsets = (
            torch.arange(batch.batch_size, device=device)
            .repeat_interleave(self.num_nodes * 2)
            * self.num_nodes * 2
        )
...
def build_cfnp_gnn_model(in_features: int, hidden_dim: int, num_obs: int, num_count: int, num_nodes: int, iterations: int = 5, mog_comp: int = 3, num_heads_att=3):
    model = GNNCFNPSeq(
